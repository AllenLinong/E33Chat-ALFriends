...
<h1 align="center">ALFriends Chat</h1>
...
ALFriends Chat is a chat-enhancement mod that rebuilds the vanilla chat HUD in a chat-app style: bubbles with heads, @ mentions, a whisper sidebar, search, emoji & quick phrases, quote reply, notification banners, local chat history, and a fully reworked settings screen.
...
1. Download the JAR for your platform from [Releases](https://github.com/E33EPUS/ALFriends Chat/releases)
...
1. Open chat and the ALFriends Chat panel appears
...
- 🧩 **Message-format templates** — The server declares its chat format so the mod can parse plugin/NCR-rewritten lines (configured via `/alfriendschat gui`)
...
- The vanilla chat box renders on the HUD as usual (shifted up 8 px clear of the ALFriends Chat icon)
...
**Path convention**: `assets/alfriendschat/textures/gui/{dark|light}/<element>.png`. Default textures are 16×16 solid-color stretch; hover/selected/close state highlights are texture-driven too (overridable since v2.2.8).
...
> History distribution and the `/tpa` switch must be enabled manually in the server config file; templates are configured via `/alfriendschat gui` or commands (or by editing the server config file directly).

---

## Message recognition

ALFriends Chat rebuilds the "who said this" layer of the chat HUD, aiming to tell player messages apart from system / broadcast ones. System-channel messages are judged in this order (first hit wins):
...
Plugins (or No Chat Reports-style no-report mods) often rewrite chat lines: title prefixes, arrows instead of colons, colored names. ALFriends Chat's built-in guards can only tell "a player is speaking" — they cannot read the exact name and body out of an unfamiliar format. **A template declares the format to the client**: where the display name, the body (and for whispers, the sender/receiver) sit. A template hit parses exactly and skips the guard chain.

### Enabling

Install ALFriends Chat on the server → after players join, an OP runs `/alfriendschat gui` to open the **server-config screen**, adds templates in the Chat Templates / Whisper Templates categories, and saves. Templates sync automatically to every client (including players who join later).
...
```
/alfriendschat template list                      # list templates
/alfriendschat template set chat <template>       # add a chat template
/alfriendschat template set whisper <template>    # add a whisper template
/alfriendschat template remove chat <index>       # remove
/alfriendschat template clear chat                # clear (fall back to guards)
/alfriendschat template test chat <index> <text>  # dry-run parse
```
...
> [!WARNING]
> Chat history is stored in plain text under `.minecraft/alfriendschat/history/` on your machine. **Do not use it on public or untrusted computers**, to avoid leaking sensitive information.
...
Client config: `config/alfriendschat-client.toml` (Fabric: `config/alfriendschat-client.json`) ｜ Server config: `saves/<world>/serverconfig/alfriendschat-server.toml` (Fabric: `.json`)

### Where is chat history stored?

`.minecraft/alfriendschat/history/`, split per world / server. Plain-text format, one line per message (`time\tsender\tcontent`), readable in any text editor; time separators gain a date across days.
...
Use **message-format templates** (2.2.6+): as an OP run `/alfriendschat gui`, paste a real chat line in the Chat Templates tab via "Generate from message…", or add a common format from the presets, then save — it syncs to the whole server. An empty template list falls back to the guards.
...
4. Back up `config/alfriendschat-client.toml`, then delete it to test for config corruption
5. Keep only ALFriends Chat to isolate mod conflicts
6. Check for custom fonts or resource packs
7. Look for `[alfriendschat]` errors in `.minecraft/logs/latest.log`
...
```bash
git clone https://github.com/E33EPUS/ALFriends Chat.git
cd ALFriends Chat
...
Open an [Issue](https://github.com/E33EPUS/ALFriends Chat/issues) and, where possible, include:

- ALFriends Chat version + Minecraft version + mod-loader version
...
- The chat sidebar includes a dedicated System channel for server and plugin notices.
- “Show System Notices in Public” is enabled by default. When enabled, notices appear in both Public and System; when disabled, they appear only in System.
- Original text styles, hover events, and click events are preserved, so plugin actions such as `[View]`, run-command links, and URLs remain interactive.

---

## Acknowledgements

Parts of the visual and interaction design of ALFriends Chat are derived from and continue the work of [E33Chat](https://github.com/E33EPUS/E33Chat).

Special thanks to **E33EPUS** for releasing E33Chat under the MIT License. ALFriends Chat builds on that foundation with ALFriends protocol integration, friend messaging, recent-conversation management, and compatibility across multiple Minecraft versions. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for the complete upstream copyright and license notice.
