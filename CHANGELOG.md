...
- **表情包 tab 缺失**：补上第三个 **Emote Pack（表情包）** tab——图片存 `runDir/alfriendschat/emotes/`（上限 10 张，png/jpg/gif），点击即上传发送、悬停 ✕ 删除、+ 号提示「拖图片进窗口添加」（拖拽导入复用现有拖拽钩子）
...
- **Missing emote-pack tab**: added the third **Emote Pack** tab — images live in `runDir/alfriendschat/emotes/` (max 10, png/jpg/gif), click to upload+send, hover ✕ to delete, the + slot shows a "drag an image onto the window to add" hint (drag-import reuses the existing drop hook)
...
- **无用 import 残留**：`ALFriends ChatCommands` 的 `ServerConfigScreenPayload` / `ServerPlayNetworking` import 未包裹 `#if MC >= 12005` 守卫；`MentionNotificationBanner` 的 `DrawContext` / `MatrixStack` import 未被使用。均已修复。
...
- **Unused import remnants**: `ALFriends ChatCommands`'s `ServerConfigScreenPayload` / `ServerPlayNetworking` imports were not wrapped in `#if MC >= 12005` guard; `MentionNotificationBanner`'s `DrawContext` / `MatrixStack` imports were unused. Both fixed.
...
- **IMBlocker 输入法适配（Fabric）**：装了 IMBlocker 后，命令输入会自动切换到英文输入法、退出命令恢复中文——与装原版聊天框的行为一致。alfriendschat 的自定义聊天框绕过了 IMBlocker 监听的 vanilla 回调，现在通过反射桥补上同一钩子；没装 IMBlocker 时完全无影响
...
- IMBlocker IME support (Fabric): with IMBlocker installed, typing a command now auto-switches the input method to English and back to Chinese on exit — matching vanilla chat. alfriendschat's custom chat screen bypasses the vanilla callback IMBlocker listens to; a reflection bridge now re-attaches the same hook, and does nothing when IMBlocker is absent
...
- **Fabric 客户端配置文件对齐**：`config/alfriendschat.json` → `config/alfriendschat-client.json`（与 Forge/Neo 的 `alfriendschat-client.toml` 对齐）；检测到旧文件自动迁移继承设置，老用户不丢配置
...
- Fabric client config file renamed `config/alfriendschat.json` → `config/alfriendschat-client.json` (aligned with Forge/Neo's `alfriendschat-client.toml`); a legacy file is auto-migrated so existing settings carry over
...
- **服务端命令走 lang 翻译**：`/alfriendschat template` 系列与模板保存校验的回复此前硬编码中文（英文客户端显示中文）；全部改为 lang 键（含测试输出/校验错误/聊天私聊类别名），系统横幅 lang 描述同步为"默认开启"
- **服务端命令走 lang 翻译**：`/alfriendschat template` 系列与模板保存校验的回复此前硬编码中文（英文客户端显示中文）；全部改为 lang 键（含测试输出/校验错误/聊天私聊类别名），系统横幅 lang 描述同步为"默认开启"
...
- **Server commands now localize**: `/alfriendschat template` replies and template-save validation were hardcoded Chinese (English clients saw Chinese); all moved to lang keys (including test output, validation errors, chat/whisper kind names), and the system-banner description now says "on by default"
- **Server commands now localize**: `/alfriendschat template` replies and template-save validation were hardcoded Chinese (English clients saw Chinese); all moved to lang keys (including test output, validation errors, chat/whisper kind names), and the system-banner description now says "on by default"
...
- 默认纹理改为 jar 内置 16×16 纯色 PNG（`assets/alfriendschat/textures/gui/{dark|light}/` 23 元素 × 2 主题），色值与原代码生成完全一致，零资源包时外观零变化
...
- 附带示例资源包 `ALFriends Chat-Texture-Demo`（已放入测试服 resourcepacks/）：气泡边框、彩色横幅等 9-slice 覆盖演示
...
- Bundled demo pack `ALFriends Chat-Texture-Demo` (placed in the test server's resourcepacks/): bubble borders, colored banners — 9-slice override demos
...
- 服务端配置（`config/alfriendschat-server.toml`）：`chat_templates` / `whisper_templates` / `template_debug`；空列表 = 关闭模板，回到守卫识别
- 游戏内命令（需 OP）：`/alfriendschat template list` / `set chat|whisper <模板>` / `remove <index>` / `clear` / `test <index> <文本>`（即时匹配预览），改完自动广播并写回 toml
...
- **服务端配置 GUI**：`/alfriendschat gui`（OP）打开图形界面，可视化编辑全部服务端配置（use_tpa / 聊天历史 / 聊天与私聊模板 / 模板诊断），保存后服务端校验、写回 toml 并广播；界面视觉对齐客户端设置（主题控件/分割线/平滑滚动）
...
- Server config (`config/alfriendschat-server.toml`): `chat_templates` / `whisper_templates` / `template_debug`; empty list = disabled, guards take over
- In-game commands (OP required): `/alfriendschat template list` / `set chat|whisper <template>` / `remove <index>` / `clear` / `test <index> <text>` (instant match preview); changes broadcast immediately and persist to the toml
...
- **Server-config GUI**: `/alfriendschat gui` (OP) opens an in-game screen to edit every server setting (use_tpa / join history / chat & whisper templates / template debug); saving validates server-side, persists to the toml and rebroadcasts; visual language matches the client settings screen (themed widgets / dividers / smooth scrolling)
...
- 纹理路径约定：`assets/alfriendschat/textures/gui/{dark|light}/panel_bg.png` 等——丢进资源包即可覆盖任意元素外观（渐变、图案、配色都可以）
...
- Path convention: `assets/alfriendschat/textures/gui/{dark|light}/panel_bg.png` etc. — drop files into a resource pack to override any element (gradients, patterns, custom colors)
...
- 兼容性红利：ChatHeads、ChatAnimation 等改造原版聊天框的 mod 自动生效（此前被 ALFriends Chat 取消渲染）
...
- Compatibility bonus: mods that restyle the vanilla chat (ChatHeads, ChatAnimation) work automatically (previously cancelled by ALFriends Chat)
...
- 表情按钮：暂不支持。Quark 表情按钮通过 Mixin 注入 `ChatScreen`，ALFriends Chat 因类加载器冲突（log4j `MessageSupplier` LinkageError）无法继承 `ChatScreen`，表情按钮需额外方案
...
- Emote buttons: not supported. Quark's emote buttons are injected via Mixin targeting `ChatScreen`; ALFriends Chat cannot extend `ChatScreen` due to a log4j classloader conflict (`MessageSupplier` LinkageError). Emote buttons require a separate integration approach.
...
**配置标题与 mod 描述修正**
- 配置界面标题从 "ChatBubble" 更正为 "ALFriends Chat"
...
- Config screen title corrected from "ChatBubble" to "ALFriends Chat"
...
**新增**
- 服务端配置 `use_tpa`（`alfriendschat-server.toml`，默认 false）：开启后右键玩家头像菜单的传送改用 `/tpa`（请求式）而非 `/tp`。设置进服时同步给客户端，菜单标签随之显示"请求传送"；未收到同步（单人/服务器没装 mod）回退 `/tp`

**New**
- Server config `use_tpa` (`alfriendschat-server.toml`, default false): when enabled, the player-head menu teleports via `/tpa` (request) instead of `/tp`. The setting is synced to clients on join and the menu label switches to "Request TP"; without a sync (singleplayer / server without the mod) it falls back to `/tp`
...
**更名**
- 显示名从 E33EPUS's ChatScreen 改为 **ALFriends Chat**

**Rename**
- Display name changed from E33EPUS's ChatScreen to **ALFriends Chat**
...
- Log prefix unified to `[alfriendschat]`
...
- jar 命名格式改为 `alfriendschat-Forge-1.20.1-1.0`
...
- Jar naming: `alfriendschat-Forge-1.20.1-1.0`
