...
import com.alinegames.alfriends.client.chat.notification.MentionNotificationBanner;
...
public class ChatBubbleHudOverlay {
    ...
    private static final int FRIEND_AVATAR_S = 20;
    private static final int FRIEND_AVATAR_GAP = 4;
    private static final int MAX_FRIEND_AVATARS = 5;
    ...
    private static Identifier chatIconTex() {
        ...
        //#if MC >= 12005
        return Identifier.of("alfriendschat", "textures/gui/" + theme + "/chat_icon.png");
        //#else
        //$$ return new Identifier("alfriendschat", "textures/gui/" + theme + "/chat_icon.png");
        //#endif
    }

    private static ChatBubbleTheme theme() {
        return ChatBubbleTheme.fromConfig(cfg().theme());
    }
    ...
    public static void render(DrawContext g) {
        ...
        renderUnreadFriendAvatars(g, iconY);
    ...
    public static ChatMessageStore.FriendContact getUnreadFriendAt(double mx, double my) {
        MinecraftClient mc = MinecraftClient.getInstance();
        if (mc.currentScreen != null) return null;
        int iconY = mc.getWindow().getScaledHeight() - ICON_S - 20;
        List<ChatMessageStore.FriendContact> unread = unreadFriends();
        int startY = iconY - unread.size() * (FRIEND_AVATAR_S + FRIEND_AVATAR_GAP);
        for (int i = 0; i < unread.size(); i++) {
            int y = startY + i * (FRIEND_AVATAR_S + FRIEND_AVATAR_GAP);
            if (mx >= 3 && mx <= 3 + FRIEND_AVATAR_S && my >= y && my <= y + FRIEND_AVATAR_S) return unread.get(i);
        }
        return null;
    }

    private static List<ChatMessageStore.FriendContact> unreadFriends() {
        return ChatMessageStore.getRecentFriendContacts().stream().filter(contact -> contact.unread() > 0 && !contact.blocked()).limit(MAX_FRIEND_AVATARS).toList();
    }

    private static void renderUnreadFriendAvatars(DrawContext g, int iconY) {
        List<ChatMessageStore.FriendContact> unread = unreadFriends();
        int startY = iconY - unread.size() * (FRIEND_AVATAR_S + FRIEND_AVATAR_GAP);
        for (int i = 0; i < unread.size(); i++) {
            ChatMessageStore.FriendContact contact = unread.get(i);
            int y = startY + i * (FRIEND_AVATAR_S + FRIEND_AVATAR_GAP);
            Identifier skin = friendSkin(contact.uuid(), contact.name());
            ColoredTextureRenderer.drawWithAlpha(g, skin, 3, y, FRIEND_AVATAR_S, FRIEND_AVATAR_S, 8.0F, 8.0F, 8, 8, 64, 64, 1.0F);
            ColoredTextureRenderer.drawWithAlpha(g, skin, 2, y - 1, FRIEND_AVATAR_S + 2, FRIEND_AVATAR_S + 2, 40.0F, 8.0F, 8, 8, 64, 64, 1.0F);
            drawScaledTip(g, 3 + FRIEND_AVATAR_S - TIP_DISP / 2, y - TIP_DISP / 2, TIP_DISP);
        }
    }

    private static Identifier friendSkin(UUID uuid, String name) {
        MinecraftClient mc = MinecraftClient.getInstance();
        if (mc.getNetworkHandler() != null && uuid != null) {
        ...
        //#if MC >= 12109
        return DefaultSkinHelper.getSkinTextures(new GameProfile(uuid, name != null ? name : "")).body().texturePath();
        //#else
        //#if MC >= 12004
