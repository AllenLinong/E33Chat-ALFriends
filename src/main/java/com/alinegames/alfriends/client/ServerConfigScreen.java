...
import com.alinegames.alfriends.client.chat.TemplateMatcher;
...
/**
 * Server-config GUI (opened by /alfriendschat gui, filled from a ServerConfigScreenPayload
...
public class ServerConfigScreen extends Screen {
    ...
    private static final String[] CHAT_PRESETS = {
        ...
        "[{prefix}] {display_name}{sep}{content}",             // TrChat default Normal channel
    ...
    private static final String[] WHISPER_PRESETS = {
        ...
        "[{sender} -> {target}] {content}",                  // TrChat default Private channel
        "[私聊] {sender}{sep}{content}",
        "{sender}私聊 {target}{sep}{content}",
    };

    private static final String[] CAT_KEYS = {
        "alfriendschat.server.cat.general",
        "alfriendschat.server.cat.chat",
        "alfriendschat.server.cat.whisper",
        "alfriendschat.server.cat.debug",
        "alfriendschat.server.cat.tutorial",
    };
    ...
    public ServerConfigScreen(Screen lastScreen, boolean useTpa, boolean history, boolean debug,
        ...
        super(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.title"));
    ...
    private void buildRows() {
        ...
        switch (selectedCat) {
            case 0 -> {
                rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.use_tpa"),
                    List.of(mkToggle(() -> useTpaV, nv -> useTpaV = nv)), null, "alfriendschat.server.use_tpa"));
                rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.history"),
                    List.of(mkToggle(() -> historyV, nv -> historyV = nv)), null, "alfriendschat.server.history"));
                rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.media"),
                    List.of(mkToggle(() -> mediaV, nv -> mediaV = nv)), null, "alfriendschat.server.media"));
                rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.media_auto_clean"),
                    List.of(mkToggle(() -> autoCleanV, nv -> autoCleanV = nv)), null, "alfriendschat.server.media_auto_clean"));
            }
            case 1 -> buildTemplateRows(chatV, true);
            case 2 -> buildTemplateRows(whisperV, false);
            case 3 -> rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.template_debug"),
                List.of(mkToggle(() -> debugV, nv -> debugV = nv)), null, "alfriendschat.server.template_debug"));
    ...
    private void buildTemplateRows(List<String> list, boolean chat) {
        ...
        for (int i = 0; i < list.size(); i++) {
            int idx = i;
            Text label = com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.template_n", i + 1);
            ...
            ButtonWidget rm = GuiCompat.button(com.alinegames.alfriends.client.Txt.literal("✕"), b -> { list.remove(idx); rebuild(); },
                previewX() - 8 - 20, 0, 20, 20);
            rows.add(new Row(label, List.of(reg(rm), reg(box)), null, ROW_H,
                "alfriendschat.server.template_n", false));
        }
        // 操作行：添加 / 从消息生成 各 90px，从右往右对齐到控件右缘线
        ButtonWidget add = GuiCompat.button(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.add"), b -> { list.add(""); rebuild(); },
            btnLeft(), 0, BUTTON_W, 20);
        if (chat) {
            ButtonWidget genOpen = GuiCompat.button(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen_open"),
                b -> { genVisible = true; rebuild(); }, btnRight(), 0, BUTTON_W, 20);
            rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.actions"), List.of(add, genOpen), null, "alfriendschat.server.actions"));
            if (genVisible) {
                TextFieldWidget genBox = mkBox(inputX(), INPUT_W);
                genBox.setText(genText);
                genBox.setChangedListener(s -> { genText = s; genError = null; });
                rows.add(genInputRow = row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen"), List.of(genBox), null, "alfriendschat.server.gen"));
                ButtonWidget genOk = GuiCompat.button(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen_confirm"),
                    b -> generateFromMessage(), btnLeft(), 0, BUTTON_W, 20);
                ButtonWidget genCancel = GuiCompat.button(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen_cancel"),
                    b -> { genVisible = false; genError = null; rebuild(); }, btnRight(), 0, BUTTON_W, 20);
                rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen"), List.of(genOk, genCancel), null, null));
            }
        } else {
            rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.actions"), List.of(add), null, "alfriendschat.server.actions"));
        }
        // 常见格式预设：标题行 + 逐行[模板字符串(标签,按按钮左缘截断)][+正方形按钮右对齐]
        String[] presets = chat ? CHAT_PRESETS : WHISPER_PRESETS;
        rows.add(titleRow(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.preset_section")));
        for (String p : presets) {
            ButtonWidget pb = GuiCompat.button(com.alinegames.alfriends.client.Txt.literal("+"),
                b -> { if (!list.contains(p)) { list.add(p); rebuild(); } },
                inputX() + INPUT_W - 20, 0, 20, 20);
            Text label = com.alinegames.alfriends.client.Txt.literal(truncate(p, inputX() - optLabelX() - 8));
        ...
        rows.add(row(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.preview"), List.of(preview),
            chat ? previewChatResult : previewWhisperResult, "alfriendschat.server.preview"));
    }

    // 教程：分节速查。每节 = 标题行 + 段落行（像素换行）+ 间距；原理节放最后（进阶）
    private void buildTutorialRows() {
        for (String key : List.of("quick", "concept", "fields", "faq", "why")) {
            rows.add(titleRow(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.tutorial." + key + ".title")));
            for (String para : com.alinegames.alfriends.client.Txt.translatable("alfriendschat.tutorial." + key).getString().split("\n")) {
                if (para.isBlank()) {
                    rows.add(textRow(com.alinegames.alfriends.client.Txt.literal(""), 6));
                    continue;
                }
                for (String line : wrapText(para)) {
                    rows.add(textRow(com.alinegames.alfriends.client.Txt.literal(line), 14));
                }
            }
            rows.add(textRow(com.alinegames.alfriends.client.Txt.literal(""), 10));
        }
    }
    ...
    private void generateFromMessage() {
        ...
        if (inferred == null) {
            ...
            genError = com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen_failed").getString()
                + "  " + com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.gen_howto").getString();
            return;
        }
        chatV.add(inferred);
        genVisible = false;
    ...
    private String runPreview(String text, List<String> raws, boolean whisper) {
        ...
        if (m.isEmpty()) return com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.preview_miss").getString();
        var t = m.orElseThrow();
        String name = whisper && t.sender() != null ? t.sender() : t.displayName();
        return com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.preview_hit", name, t.content()).getString();
    }
    ...
    private TextFieldWidget mkBox(int x, int w) {
        TextFieldWidget box = new TextFieldWidget(textRenderer, x, 0, w, 20, com.alinegames.alfriends.client.Txt.literal(""));
    ...
    private static String validate(String kind, List<String> templates) {
        for (int i = 0; i < templates.size(); i++) {
            ...
            if (result.template() == null) {
                return com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.invalid",
                    com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server." + kind), i + 1, result.error()).getString();
            }
        }
        return null;
    }
    ...
    //#endif
        if (changed()) {
            GuiCompat.setScreen(client, new ConfirmScreen(confirmed -> {
                ...
                com.alinegames.alfriends.client.Txt.translatable("alfriendschat.config.discard.message", changeCount())));
        } else {
            doClose();
        }
    }
    ...
    protected void init() {
        ...
        exitBtn = GuiCompat.addDrawableChild(this, GuiCompat.button(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.config.exit"), b -> doClose(),
            width / 2 - 104, height - 32, 100, 20));
        saveBtn = GuiCompat.addDrawableChild(this, GuiCompat.button(com.alinegames.alfriends.client.Txt.translatable("alfriendschat.server.save"), b -> save(),
    ...
    //#endif
        ...
        for (int i = 0; i < CAT_KEYS.length; i++) {
            ...
            RenderHelper.drawText(g, textRenderer, com.alinegames.alfriends.client.Txt.translatable(CAT_KEYS[i]), CAT_X + 18, ly + (CAT_ROW_H - 8) / 2,
        ...
        for (Row row : rows) {
            ...
            if (row.extraText() != null && !row.extraText().isEmpty()) {
                RenderHelper.drawText(g, textRenderer, com.alinegames.alfriends.client.Txt.literal(truncate(row.extraText(), rightAreaW())),
            ...
            for (Element w : row.widgets()) {
                if (w instanceof TextFieldWidget eb && boxErrors.containsKey(eb)) {
                    RenderHelper.drawText(g, textRenderer, com.alinegames.alfriends.client.Txt.literal(truncate(boxErrors.get(eb), rightAreaW())),
            ...
            if (genError != null && row == genInputRow) {
                RenderHelper.drawText(g, textRenderer, com.alinegames.alfriends.client.Txt.literal(truncate(genError, rightAreaW())),
                    optLabelX(), y + 22, 0xFFFF4444, false);
            }
            y += row.height();
        }
        ...
        if (error != null) {
            ...
            RenderHelper.drawText(g, textRenderer, com.alinegames.alfriends.client.Txt.literal(truncate(error, rightAreaW())),
                optLabelX(), viewBottom() - 12, 0xFFFF4444, false);
        }

        if (tooltipKey != null)
            GuiCompat.renderTooltip(g, this, com.alinegames.alfriends.client.Txt.translatable(tooltipKey + ".desc"), mouseX, mouseY);
    }
