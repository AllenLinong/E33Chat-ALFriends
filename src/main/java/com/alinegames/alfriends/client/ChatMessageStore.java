...
import com.alinegames.alfriends.client.chat.notification.MentionNotificationController;
...
public class ChatMessageStore {
    ...
    public static void syncFriendContacts(List<ALFriendsContactsPayload.Contact> contacts) {
        Map<UUID, FriendContact> updated = new LinkedHashMap<>();
        for (ALFriendsContactsPayload.Contact contact : contacts) {
        ...
        friendContacts.putAll(updated);
    }

    public static void clearFriendContacts() {
        friendContacts.clear();
        hiddenRecentContacts.clear();
        openWhisperPartner = null;
    }

    public static void rememberFriend(UUID uuid, String name) {
        if (uuid == null || name == null || name.isBlank()) return;
        FriendContact existing = friendContacts.get(uuid);
        if (existing == null) {
            friendContacts.put(uuid, new FriendContact(uuid, name, 0, 0L, false, false));
        } else if (!existing.name().equals(name)) {
            friendContacts.put(uuid, new FriendContact(uuid, name, existing.unread(), existing.lastMessageAt(), existing.friend(), existing.blocked()));
        }
        rememberPlayer(uuid, name, name);
    }

    public static void noteFriendMessage(UUID uuid, String name, long sentAt, boolean outgoing) {
        if (uuid == null || name == null || name.isBlank()) return;
        FriendContact existing = friendContacts.get(uuid);
        int unread = existing == null ? 0 : existing.unread();
        if (!outgoing && !isOpenWhisper(name)) unread++;
        long lastMessageAt = Math.max(sentAt, existing == null ? 0L : existing.lastMessageAt());
        friendContacts.put(uuid, new FriendContact(uuid, name, unread, lastMessageAt,
            existing != null && existing.friend(), existing != null && existing.blocked()));
        hiddenRecentContacts.remove(uuid);
        rememberPlayer(uuid, name, name);
    }

    public static List<FriendContact> getFriendContacts() {
        List<FriendContact> contacts = new ArrayList<>(friendContacts.values());
        contacts.removeIf(contact -> !contact.friend() || contact.blocked());
        sortContacts(contacts);
        return contacts;
    }

    public static List<FriendContact> getRecentFriendContacts() {
        List<FriendContact> contacts = new ArrayList<>(friendContacts.values());
        contacts.removeIf(contact -> contact.lastMessageAt() <= 0L || hiddenRecentContacts.contains(contact.uuid()));
        sortContacts(contacts);
        return contacts;
    }

    private static void sortContacts(List<FriendContact> contacts) {
        contacts.sort(Comparator
            .comparingInt((FriendContact contact) -> contact.unread() > 0 ? 0 : 1)
            .thenComparing(Comparator.comparingLong(FriendContact::lastMessageAt).reversed())
            .thenComparing(FriendContact::name, String.CASE_INSENSITIVE_ORDER));
    }

    public static void activateFriendConversation(FriendContact contact) {
        if (contact == null) return;
        long lastMessageAt = Math.max(System.currentTimeMillis(), contact.lastMessageAt());
        friendContacts.put(contact.uuid(), new FriendContact(
            contact.uuid(), contact.name(), contact.unread(), lastMessageAt, contact.friend(), contact.blocked()));
        hiddenRecentContacts.remove(contact.uuid());
    }

    public static void removeRecentFriend(String playerName) {
        if (playerName == null || playerName.isBlank()) return;
        friendContacts.values().stream()
            .filter(contact -> contact.name().equalsIgnoreCase(playerName))
            .findFirst()
            .ifPresent(contact -> hiddenRecentContacts.add(contact.uuid()));
    }

    public static boolean isFriendContact(String playerName) {
        if (playerName == null || playerName.isBlank()) return false;
        return friendContacts.values().stream().filter(contact -> contact.friend() && !contact.blocked())
            .anyMatch(contact -> contact.name().equalsIgnoreCase(playerName));
    }

    public static void setOpenWhisperPartner(String partner) {
        openWhisperPartner = partner;
        if (partner != null) clearUnreadWhisper(partner);
    }

    private static boolean isOpenWhisper(String partner) {
        return screenOpen && partner != null && partner.equalsIgnoreCase(openWhisperPartner);
    }

    // Server-synced setting: head-menu teleport uses /tpa instead of /tp (default true)
    private static volatile boolean serverUseTpa = true;
    ...
    private static volatile List<com.alinegames.alfriends.client.chat.TemplateMatcher.CompiledTemplate> serverChatTemplates = List.of();
    private static volatile List<com.alinegames.alfriends.client.chat.TemplateMatcher.CompiledTemplate> serverWhisperTemplates = List.of();
    ...
    private static List<com.alinegames.alfriends.client.chat.TemplateMatcher.CompiledTemplate> compileTemplates(List<String> raws) {
        if (raws == null || raws.isEmpty()) return List.of();
        List<com.alinegames.alfriends.client.chat.TemplateMatcher.CompiledTemplate> out = new ArrayList<>();
        for (String raw : raws) {
            var result = com.alinegames.alfriends.client.chat.TemplateMatcher.compile(raw);
            if (result.template() != null) {
                out.add(result.template());
                if (!result.template().unknownFields().isEmpty()) {
                    debugLog(() -> "[alfriendschat] template has unknown placeholders (treated as literal): "
                        + result.template().unknownFields() + " | template='" + raw + "'");
                }
            } else {
                debugLog(() -> "[alfriendschat] server template skipped: '" + raw + "' -> " + result.error());
            }
        }
        return out;
    }

    public static List<com.alinegames.alfriends.client.chat.TemplateMatcher.CompiledTemplate> serverChatTemplates() {
        return serverChatTemplates;
    }

    public static List<com.alinegames.alfriends.client.chat.TemplateMatcher.CompiledTemplate> serverWhisperTemplates() {
        return serverWhisperTemplates;
    }

    public static boolean serverTemplateDebug() { return serverTemplateDebug; }
    public static void clearServerPlayerPresentation() {
        seenPlayers.clear();
        pendingMetas.clear();
    ...
    public static boolean isPlayerBlocked(String rawPlayerName, Text senderName, List<? extends String> blocked) {
        if (ALFriendsBridge.active()) {
            return friendContacts.values().stream().anyMatch(contact -> contact.blocked()
                && (contact.name().equalsIgnoreCase(rawPlayerName)
                    || (senderName != null && contact.name().equalsIgnoreCase(senderName.getString()))));
        }
    ...
    // Blocking only affects messages received after the block. Existing history
    // remains visible so blocking a player never rewrites the conversation.
    ...
    public static void addMessage(Text content, UUID senderUUID, Text senderName, boolean isSystem, String rawPlayerName, boolean whisper, String whisperPartner, boolean localSend) {
        ...
        boolean hasRawPlayerName = rawPlayerName != null && !rawPlayerName.isEmpty();
        // Some offline-mode servers/proxies reuse or zero player UUIDs. When the
        // parser has a real profile name, the exact bare name must take priority;
        // otherwise a nearby player can inherit the local UUID and render on the
        // right as if it were our own message.
        boolean own = hasRawPlayerName
            ? rawPlayerName.equals(playerName)
            : localPlayer != null && senderUUID != null && senderUUID.equals(localPlayer.getUuid());
        if (!hasRawPlayerName && !own && senderName != null) {
            own = senderName.getString().equals(playerName);
        }
        if (!own && !isSystem && isPlayerBlocked(rawPlayerName, senderName,
        ...
        boolean isMentionOrQuote = !isSystem
            && com.alinegames.alfriends.client.chat.MentionDetector.isMentioned(
        ...
        // System messages remain in the system channel; they do not create a top-screen banner.
    ...
    public static boolean isSystemMessage(ChatMessage msg) {
        if (msg == null || msg.whisper()) return false;
        if (msg.isSystem()) return true;
        UUID sender = msg.senderUUID();
        boolean noPlayerIdentity = sender == null || sender.equals(new UUID(0, 0));
        return noPlayerIdentity && (msg.rawPlayerName() == null || msg.rawPlayerName().isBlank());
    }

    public static List<ChatMessage> getSystemMessages() {
        List<ChatMessage> result = new ArrayList<>();
        for (ChatMessage msg : messages) {
            if (isSystemMessage(msg)) result.add(msg);
        }
        return result;
    }
    ...
    public static void markWhisperUnread(String partner) {
        if (partner == null || isOpenWhisper(partner)) return;
        for (Map.Entry<UUID, FriendContact> entry : friendContacts.entrySet()) {
            ...
            if (contact.name().equalsIgnoreCase(partner)) {
                entry.setValue(new FriendContact(contact.uuid(), contact.name(),
                    Math.max(1, contact.unread()), contact.lastMessageAt(), contact.friend(), contact.blocked()));
                return;
            }
        }
    }
    ...
    public static boolean hasUnreadWhisper(String partner) {
        ...
        return friendContacts.values().stream()
            .anyMatch(contact -> contact.name().equalsIgnoreCase(partner) && contact.unread() > 0);
    }

    public static int getUnreadCount() {
        return friendContacts.values().stream().mapToInt(FriendContact::unread).sum();
    }
    ...
    private static File getHistoryFile(String worldKey) {
        ...
        return new File(MinecraftClient.getInstance().runDirectory,
            "alfriendschat/history/" + safe + "_" + sha256Short(worldKey) + ".json");
    }
    ...
    private static File getLegacyHistoryFile(String worldKey) {
        ...
        return new File(MinecraftClient.getInstance().runDirectory,
            "alfriendschat/history/" + safe + "_" + hash + ".json");
    }
    ...
    private static List<ChatMessage> loadLegacyFile(File f) {
        ...
        try (Reader r = new InputStreamReader(new FileInputStream(f), StandardCharsets.UTF_8)) {
            ...
            for (int i = list.size() - 1; i >= 0; i--) {
                ...
                } catch (Exception e) { E33Log.warn("[alfriendschat] Failed to read/write chat history", e); }
            }
        } catch (Exception e) { E33Log.warn("[alfriendschat] Failed to read/write chat history", e); }
        return out;
    }
    ...
    private static final java.util.concurrent.ExecutorService SAVE_EXECUTOR =
        java.util.concurrent.Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r, "alfriendschat-history-save");
            t.setDaemon(true);
            return t;
        });
    ...
    private static void saveMessages(String worldKey) {
        ...
        SAVE_EXECUTOR.execute(() -> {
            ...
            try (Writer w = new OutputStreamWriter(new FileOutputStream(tmp), StandardCharsets.UTF_8)) {
                ...
                E33Log.warn("[alfriendschat] Failed to read/write chat history", e);
                return;
            }
            try {
                ...
                try {
                    ...
                    E33Log.warn("[alfriendschat] Failed to read/write chat history", e2);
                }
            }
        });
    }
    ...
    private static void cleanupOldHistory() {
        ...
        if (days <= 0) return;
        File dir = new File(MinecraftClient.getInstance().runDirectory, "alfriendschat/history");
        ...
        for (File f : files) {
            ...
            if (isExpired(f.lastModified(), now, days)) {
                E33Log.info("[alfriendschat] History retention: deleting " + f.getName());
                f.delete();
            }
        }
    }
    ...
    public static void addHistoryMessages(List<com.alinegames.alfriends.client.network.HistoryPayload.HistoryEntry> entries) {
    ...
    public static void syncALFriendsHistory(
            com.alinegames.alfriends.client.network.ALFriendsHistoryPayload payload) {
        rememberPlayer(payload.partner(), payload.partnerName(), payload.partnerName());
        boolean firstChunk = messages.stream().noneMatch(message -> message.whisper()
            && payload.partnerName().equals(message.whisperPartner()));
        var player = net.minecraft.client.MinecraftClient.getInstance().player;
        UUID ownUuid = player == null ? new UUID(0, 0) : player.getUuid();
        for (var entry : payload.entries()) {
            boolean own = ownUuid.equals(entry.sender());
            ChatMessage synced = new ChatMessage(entry.sender(), net.minecraft.text.Text.literal(entry.senderName()),
                net.minecraft.text.Text.literal(entry.content()), entry.sentAt(), own, false,
                entry.quoteContent().isEmpty() ? null : entry.quoteContent(),
                entry.quoteSender().isEmpty() ? null : entry.quoteSender(),
                String.valueOf(entry.content().hashCode()), 1, entry.senderName(), true, payload.partnerName());
            if (firstChunk || messages.stream().noneMatch(existing -> existing.time() == synced.time()
                    && existing.senderUUID().equals(synced.senderUUID())
                    && existing.content().getString().equals(synced.content().getString()))) {
                messages.add(synced);
            }
            rememberPlayer(entry.sender(), entry.senderName(), entry.senderName());
        }
        while (messages.size() > MAX) messages.remove(0);
    }
