...
public class RoundRectRenderer {
    ...
    private static final int DEFAULT_ANTIALIASING = 4;
    ...
    public static void fill(Object g, int x1, int y1, int x2, int y2, float radius, int argb) {
        ...
        int supersample = sampleCount();

        // Draw at a higher logical resolution, then scale the result back down.
        // DrawContext.fill is quantized to one GUI pixel, which becomes a visible
        // stair step at larger GUI scales. Eight sub-pixels per GUI pixel preserve
        // the safe DrawContext pipeline while smoothing the rounded boundary.
        RenderHelper.pushMatrix(g);
        RenderHelper.translate(g, x1, y1);
        float inverseScale = 1.0F / supersample;
        RenderHelper.scale(g, inverseScale, inverseScale);
        scanLineFill(g, 0, 0, w * supersample, h * supersample,
            radius * supersample, argb);
        RenderHelper.popMatrix(g);
    }

    private static int sampleCount() {
        int quality = DEFAULT_ANTIALIASING;
        try {
            Integer configured = ChatBubbleClientSetup.config().bubbleAntialiasing();
            if (configured != null) quality = Math.max(0, Math.min(10, configured));
