...
public record ServerConfigSavePayload(boolean useTpa, boolean historyEnabled, boolean templateDebug,
    ...
    public static final CustomPayload.Id<ServerConfigSavePayload> ID =
        new CustomPayload.Id<>(
            //#if MC >= 12000
            Identifier.of("alfriendschat", "server_config_save")
            //#else
            //$$ new Identifier("alfriendschat", "server_config_save")
            //#endif
        );
    ...
    //#else
    //$$ public static final Identifier ID = new Identifier("alfriendschat", "server_config_save");
    //#endif
    ...
    public static void handleServer(ServerConfigSavePayload payload, ServerPlayerEntity player,
        ...
        if (error != null) {
            //#if MC >= 26000
            //$$ player.sendSystemMessage(Text.translatable("alfriendschat.server.save_failed", error)
            //$$     .formatted(Formatting.RED));
            //#else
            player.sendMessage(Text.translatable("alfriendschat.server.save_failed", error)
        ...
        //#if MC >= 26000
        //$$ player.sendSystemMessage(Text.translatable("alfriendschat.server.saved"));
        //#else
        player.sendMessage(Text.translatable("alfriendschat.server.saved"), false);
        //#endif
    }
    ...
    private static Text validateTemplates(boolean chat, List<String> templates) {
        for (int i = 0; i < templates.size(); i++) {
            ...
            if (result.template() == null) {
                return Text.translatable("alfriendschat.server.template_invalid",
                    Text.translatable(chat ? "alfriendschat.server.kind_chat" : "alfriendschat.server.kind_whisper"),
                    i + 1, result.error());
            }
        }
        return null;
