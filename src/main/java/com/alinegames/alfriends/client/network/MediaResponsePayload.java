...
public record MediaResponsePayload(String mediaId, int index, int totalChunks, byte[] chunk)
    ...
    public static final CustomPayload.Id<MediaResponsePayload> ID =
        new CustomPayload.Id<>(
            //#if MC >= 12000
            Identifier.of("alfriendschat", "media_response")
            //#else
            //$$ new Identifier("alfriendschat", "media_response")
            //#endif
        );
    ...
    //#else
    //$$ public static final Identifier ID = new Identifier("alfriendschat", "media_response");
    //#endif
}
