 ...
 * snapshot. Triggered by /alfriendschat gui (OP). The handler (opening the client
...
public record ServerConfigScreenPayload(boolean useTpa, boolean historyEnabled, boolean templateDebug,
    ...
    public static final CustomPayload.Id<ServerConfigScreenPayload> ID =
        new CustomPayload.Id<>(
            //#if MC >= 12000
            Identifier.of("alfriendschat", "server_config_screen")
            //#else
            //$$ new Identifier("alfriendschat", "server_config_screen")
            //#endif
        );
    ...
    //#else
    //$$ public static final Identifier ID = new Identifier("alfriendschat", "server_config_screen");
    //#endif
}
