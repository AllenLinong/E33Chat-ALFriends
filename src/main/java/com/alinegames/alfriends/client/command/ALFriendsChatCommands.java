package com.alinegames.alfriends.client.command;
//#if MC >= 11900
import com.alinegames.alfriends.client.ChatBubbleMod;
...
/** Server-side management of message-format templates (/alfriendschat template ...). */
public class ALFriendsChatCommands {
    ...
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            ...
            dispatcher.register(net.minecraft.server.command.CommandManager.literal("alfriendschat")
    ...
    private static int openServerGui(ServerCommandSource src) {
        ...
        if (player == null) {
            src.sendError(Text.translatable("alfriendschat.server.console_only"));
            return 0;
        }
        //#if MC >= 12005
    ...
    private static int list(ServerCommandSource src) {
        feedback(src, Text.translatable("alfriendschat.server.tpl_list_chat_header", templates(true).size()), false);
        printTemplates(src, templates(true));
        feedback(src, Text.translatable("alfriendschat.server.tpl_list_whisper_header", templates(false).size()), false);
    ...
    private static void printTemplates(ServerCommandSource src, List<String> templates) {
        if (templates.isEmpty()) {
            feedback(src, Text.translatable("alfriendschat.server.tpl_list_empty"), false);
            return;
        }
        int i = 1;
    ...
    private static int set(ServerCommandSource src, boolean chat, String raw) {
        ...
        if (result.template() == null) {
            src.sendError(Text.translatable("alfriendschat.server.tpl_set_invalid", result.error()));
            return 0;
        }
        if (!result.template().unknownFields().isEmpty()) {
            feedback(src, Text.translatable("alfriendschat.server.tpl_set_unknown_fields", result.template().unknownFields()), false);
        }
        List<String> next = new ArrayList<>(templates(chat));
        if (next.contains(raw)) {
            src.sendError(Text.translatable("alfriendschat.server.tpl_set_duplicate"));
            return 0;
        }
        next.add(raw);
        updateTemplates(src, chat, next);
        feedback(src, Text.translatable("alfriendschat.server.tpl_set_added", next.size(), raw), false);
        return 1;
    }
    ...
    private static int remove(ServerCommandSource src, boolean chat, int index) {
        ...
        if (index < 1 || index > next.size()) {
            src.sendError(Text.translatable("alfriendschat.server.tpl_remove_bad_index", next.size()));
            return 0;
        }
        String removed = next.remove(index - 1);
        updateTemplates(src, chat, next);
        feedback(src, Text.translatable("alfriendschat.server.tpl_remove_done", removed), false);
        return 1;
    }

    private static int clear(ServerCommandSource src, boolean chat) {
        updateTemplates(src, chat, List.of());
        feedback(src, Text.translatable("alfriendschat.server.tpl_clear_done",
            Text.translatable(chat ? "alfriendschat.server.kind_chat" : "alfriendschat.server.kind_whisper")), false);
        return 1;
    }
    ...
    private static int test(ServerCommandSource src, boolean chat, int index, String text) {
        ...
        if (index < 1 || index > raws.size()) {
            src.sendError(Text.translatable("alfriendschat.server.tpl_test_bad_index", raws.size()));
            return 0;
        }
        TemplateMatcher.CompileResult result = TemplateMatcher.compile(raws.get(index - 1));
        if (result.template() == null) {
            src.sendError(Text.translatable("alfriendschat.server.tpl_test_unparseable", result.error()));
            return 0;
        }
        ...
        if (match.isEmpty()) {
            feedback(src, Text.translatable("alfriendschat.server.tpl_test_no_match"), false);
            return 1;
        }
        var r = match.orElseThrow();
        feedback(src, Text.translatable("alfriendschat.server.tpl_test_matched",
                Text.translatable(whisper ? "alfriendschat.server.kind_whisper" : "alfriendschat.server.kind_chat")), false);
        if (r.prefix() != null) feedback(src, Text.translatable("alfriendschat.server.tpl_test_field_prefix", r.prefix()), false);
        if (r.displayName() != null) {
            feedback(src, Text.translatable("alfriendschat.server.tpl_test_field_name",
                    whisper ? "sender/target" : "display_name", r.displayName())
                .copy().append(Text.translatable("alfriendschat.server.tpl_test_verified")), false);
        }
        if (r.sender() != null && r.target() != null) {
            feedback(src, Text.translatable("alfriendschat.server.tpl_test_field_sender", r.sender(), r.target()), false);
        }
        feedback(src, Text.translatable("alfriendschat.server.tpl_test_field_content", r.content()), false);
        feedback(src, Text.translatable("alfriendschat.server.tpl_test_field_offset",
