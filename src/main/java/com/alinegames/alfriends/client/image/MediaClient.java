 ...
 * (alfriendschat://media/<id>, permanent) instead of the third-party host; when
...
public final class MediaClient {
     ...
     * alfriendschat://media/<id> URL, or null on any failure (caller falls back).
     */
    public static String upload(byte[] bytes, String contentType) {
        ...
        try {
            ...
            return mediaId != null ? "alfriendschat://media/" + mediaId : null;
        } catch (Exception e) {
            UPLOADS.remove(uploadId);
            LOGGER.info("[alfriendschat] server media upload timed out after {}s", TIMEOUT_SECONDS);
            return null;
        }
    }
    ...
    public static byte[] fetch(String mediaId) {
        ...
        try {
            ...
            LOGGER.info("[alfriendschat] server media fetch {} timed out after {}s", mediaId, TIMEOUT_SECONDS);
            return null;
        }
    }
    //#else
