 ...
 *    their GPU textures.
 *  - decoded textures retain useful source resolution for crisp chat display
 *    and preview, with a bounded maximum dimension for GPU safety.
 */
public final class ImageLoader {
    ...
    private static final java.util.concurrent.ExecutorService EXEC =
        java.util.concurrent.Executors.newFixedThreadPool(2, r -> {
            Thread t = new Thread(r, "alfriendschat-image");
            t.setDaemon(true);
            return t;
        });
    ...
    static final int MAX_TEXTURE_DIMENSION = 2048;
        ...
        if (!e) {
            MinecraftClient.getInstance().execute(() -> {
                ...
                for (String u : CACHE.keySet()) {
                    tm.destroyTexture(Identifier.of("alfriendschat", "img/" + hash(u)));
                }
                CACHE.clear();
                LRU.clear();
                PENDING.clear();
    ...
    public static boolean isUsableUrl(String url) {
        ...
        if (lower.startsWith("alfriendschat://")) {
            // Server-hosted media: alfriendschat://media/<32-hex id>
            return lower.startsWith("alfriendschat://media/")
                && com.alinegames.alfriends.client.server.DiskMediaStore.isValidMediaId(
                    url.substring("alfriendschat://media/".length()));
        }
    ...
    public static int[] scaledSize(int w, int h) {
        ...
        if (w <= MAX_TEXTURE_DIMENSION && h <= MAX_TEXTURE_DIMENSION) return new int[]{w, h};
        double scale = Math.min((double) MAX_TEXTURE_DIMENSION / w,
            (double) MAX_TEXTURE_DIMENSION / h);
        return new int[]{
    ...
    private static void launchFetch(String url, ImageEntry entry) {
        CompletableFuture.runAsync(() -> fetchAndDecode(url, entry), EXEC)
            ...
            .exceptionally(t -> {
                ...
                LOGGER.info("[alfriendschat] image fetch {} -> future timeout after {}s (download may still finish)",
    ...
    private static void fetchAndDecode(String url, ImageEntry entry) {
        ...
        try {
            byte[] body;
            long t1;
            if (url.startsWith("alfriendschat://")) {
                body = MediaClient.fetch(url.substring("alfriendschat://media/".length()));
                t1 = System.currentTimeMillis();
                if (body == null) {
                    entry.markFailed("server media fetch failed");
                    LOGGER.info("[alfriendschat] image fetch {} -> server media fetch failed ({}ms)",
                        url, t1 - t0);
                    return;
                }
            } else {
                ...
                if (resp.statusCode() < 200 || resp.statusCode() >= 300) {
                    ...
                    LOGGER.info("[alfriendschat] image fetch {} -> HTTP {} ({}ms)", url, resp.statusCode(), t1 - t0);
                    return;
                }
                body = resp.body();
                if (body == null || body.length == 0 || body.length > MAX_RECEIVE_BYTES) {
                    entry.markFailed("empty or too large");
                    LOGGER.info("[alfriendschat] image fetch {} -> bad body {} bytes ({}ms)", url, body == null ? 0 : body.length, t1 - t0);
                    return;
                }
            }
            ...
            if (decoded == null) {
                ...
                LOGGER.info("[alfriendschat] image fetch {} -> decode failed ({} bytes, {}ms)", url, body.length, t1 - t0);
                return;
            }
            // Retain source detail for 1:1 chat rendering and the full-screen
            // viewer. Only exceptionally large textures are reduced.
            ...
            if (sc[0] != decoded.width() || sc[1] != decoded.height()) {
                ...
                try {
                    ...
                    LOGGER.info("[alfriendschat] image fetch {} -> scale failed: {}", url, t.toString());
                    return;
                }
                decoded.image().close();
                decoded = new RasterImageDecoder.DecodedImage(scaled, sc[0], sc[1]);
            }
            LOGGER.info("[alfriendschat] image fetch {} -> {}x{} ({} bytes, {}ms)",
            ...
            MinecraftClient.getInstance().execute(() -> {
                if (entry.state() != ImageEntry.State.LOADING) {
                    ...
                    LOGGER.info("[alfriendschat] image upload SKIPPED (state {}) for {}", entry.state(), url);
                    return;
                }
                ...
                try {
                    Identifier id = Identifier.of("alfriendschat", "img/" + hash(url));
                    TextureManager tm = MinecraftClient.getInstance().getTextureManager();
                    ...
                    LOGGER.info("[alfriendschat] image upload OK {} -> {}x{} @ {}", url, entry.width(), entry.height(), id);
                } catch (Throwable t) {
                    entry.markFailed("upload: " + t);
                    LOGGER.info("[alfriendschat] image upload FAILED {}: {}", url, t.toString());
                }
            });
            ...
            LOGGER.info("[alfriendschat] image fetch {} -> exception ({}ms): {}", url, t1 - t0, t.toString());
