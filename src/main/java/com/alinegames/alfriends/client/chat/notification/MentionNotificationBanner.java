...
import com.alinegames.alfriends.client.Animation;
import com.alinegames.alfriends.client.AnimationStyle;
import com.alinegames.alfriends.client.BlurRenderer;
import com.alinegames.alfriends.client.ChatBubbleClientSetup;
import com.alinegames.alfriends.client.ChatMessageStore;
import com.alinegames.alfriends.client.RenderHelper;
import com.alinegames.alfriends.client.RoundRectRenderer;
...
public class MentionNotificationBanner {
    ...
    public void enqueue(UUID senderUUID, Text senderName, Text content,
        ...
        String prefix = switch (type) {
            case MENTION -> com.alinegames.alfriends.client.Txt.translatable("alfriendschat.banner.mention").getString();
            case QUOTE -> com.alinegames.alfriends.client.Txt.translatable("alfriendschat.banner.quote").getString();
            case WHISPER -> com.alinegames.alfriends.client.Txt.translatable("alfriendschat.banner.whisper").getString();
            case SYSTEM -> com.alinegames.alfriends.client.Txt.translatable("alfriendschat.banner.system").getString();
        };
        Text labeledName = com.alinegames.alfriends.client.Txt.literal(prefix).append(senderName);
        ...
        if (nameLines.isEmpty()) {
            ...
            nameSeq = mc.textRenderer.wrapLines(com.alinegames.alfriends.client.Txt.literal(plainName), maxTextW).get(0);
        } else {
    ...
    public void tick() {
        ...
        if (state != prev) {
            ...
            ChatMessageStore.debugLog(() -> "[alfriendschat] Banner " + prev + " -> "
    ...
    public void render(Object g, int screenW, int screenH) {
        ...
        var theme = com.alinegames.alfriends.client.ChatBubbleTheme.valueOf(
    ...
    private static Text truncateStyled(Text src, int maxWidth,
        ...
        MutableText out = com.alinegames.alfriends.client.Txt.empty();
        int[] used = {0};
        src.visit((style, text) -> {
            if (used[0] >= budget) return java.util.Optional.<Object>empty();
            int w = font.getWidth(text);
            if (used[0] + w <= budget) {
                out.append(com.alinegames.alfriends.client.Txt.literal(text).fillStyle(style));
                used[0] += w;
            } else {
                String sub = font.trimToWidth(text, budget - used[0]);
                out.append(com.alinegames.alfriends.client.Txt.literal(sub).fillStyle(style));
                used[0] = budget;
            }
            return java.util.Optional.<Object>empty();
        }, net.minecraft.text.Style.EMPTY);
        return out.append(com.alinegames.alfriends.client.Txt.literal(suffix));
    }
