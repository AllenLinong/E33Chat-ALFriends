...
class TemplateMatcherTest {
    ...
    @Test void matchesTrChatDefaultNormalChannel() {
        var r = matchChat("[world] Steve: hello", "[{prefix}] {display_name}{sep}{content}").orElseThrow();
        assertEquals("world", r.prefix());
        assertEquals("Steve", r.displayName());
        assertEquals("hello", r.content());
    }

    @Test void matchesTrChatDefaultPrivateChannel() {
        var r = match("[Alex -> Steve] hello", "[{sender} -> {target}] {content}", null).orElseThrow();
        assertEquals("Alex", r.sender());
        assertEquals("Steve", r.target());
        assertEquals("hello", r.content());
