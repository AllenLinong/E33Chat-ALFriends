...
class ChatMessageStoreTest {
    ...
    @Test void recentContacts_onlyIncludeStartedConversations() {
        UUID newFriend = UUID.randomUUID();
        UUID recentFriend = UUID.randomUUID();
        ChatMessageStore.rememberFriend(newFriend, "NewFriend");
        ChatMessageStore.noteFriendMessage(recentFriend, "RecentFriend", 100L, true);

        assertEquals(List.of("RecentFriend"), ChatMessageStore.getRecentFriendContacts().stream()
            .map(ChatMessageStore.FriendContact::name).toList());
        assertEquals(2, ChatMessageStore.getFriendContacts().size());
    }

    @Test void removingRecentContact_doesNotDeleteFriend() {
        UUID friendId = UUID.randomUUID();
        ChatMessageStore.noteFriendMessage(friendId, "Steve", 100L, true);

        ChatMessageStore.removeRecentFriend("Steve");

        assertTrue(ChatMessageStore.getRecentFriendContacts().isEmpty());
        assertTrue(ChatMessageStore.isFriendContact("Steve"));
    }

    @Test void newMessage_restoresRemovedRecentContact() {
        UUID friendId = UUID.randomUUID();
        ChatMessageStore.noteFriendMessage(friendId, "Alex", 100L, true);
        ChatMessageStore.removeRecentFriend("Alex");

        ChatMessageStore.noteFriendMessage(friendId, "Alex", 200L, false);

        assertEquals("Alex", ChatMessageStore.getRecentFriendContacts().get(0).name());
    }

    @Test void systemChannel_detectsGrayAndBubbleSystemNotices() {
        var grayNotice = new ChatMessageStore.ChatMessage(
            new UUID(0, 0), net.minecraft.text.Text.literal("System"),
            net.minecraft.text.Text.literal("Restarting"), 100L, false, true,
            null, null, "", 1, null, false, null);
        var bubbleNotice = new ChatMessageStore.ChatMessage(
            new UUID(0, 0), net.minecraft.text.Text.literal("System"),
            net.minecraft.text.Text.literal("Restarting"), 100L, false, false,
            null, null, "", 1, null, false, null);
        var unresolvedPlayer = new ChatMessageStore.ChatMessage(
            new UUID(0, 0), net.minecraft.text.Text.literal("Steve"),
            net.minecraft.text.Text.literal("Hello"), 100L, false, false,
            null, null, "", 1, "Steve", false, null);

        assertTrue(ChatMessageStore.isSystemMessage(grayNotice));
        assertTrue(ChatMessageStore.isSystemMessage(bubbleNotice));
        assertFalse(ChatMessageStore.isSystemMessage(unresolvedPlayer));
    }

    @Test void systemChannel_keepsClickableNoticeStyle() {
        org.junit.jupiter.api.Assumptions.assumeTrue(
            net.minecraft.client.MinecraftClient.getInstance() != null,
            "click-event construction requires a running Minecraft client");
        var click = new net.minecraft.text.ClickEvent(
            net.minecraft.text.ClickEvent.Action.RUN_COMMAND, "/help");
        var content = net.minecraft.text.Text.literal("[查看]")
            .styled(style -> style.withClickEvent(click));
        var notice = new ChatMessageStore.ChatMessage(
            new UUID(0, 0), net.minecraft.text.Text.literal("System"), content,
            100L, false, true, null, null, "", 1, null, false, null);

        assertTrue(ChatMessageStore.isSystemMessage(notice));
        assertNotNull(notice.content().getStyle().getClickEvent());
    }
    ...
    @Test void wholeName_suffixExtensionMisses() {
        ...
        assertFalse(ChatMessageStore.containsWholeName("dyhuo_feng", "huo_feng"));
        assertTrue(ChatMessageStore.containsWholeName("<dyhuo_feng>: hello", "dyhuo_feng"));
    }
